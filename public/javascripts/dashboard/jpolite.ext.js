...
function myLiveEvents(){
	...
	$(".actionMin").live("click", function(){
		$(this).parents(".module")[0].min();
		$(this).hide().siblings(".actionMax").show();
	});
	$(".actionMax").live("click", function(){
		...
		buttons: {
			确定: function() {
				...
				attrs.each(function(index,input){
					if(tmp.length>1)
						tmp+=',';
                    var attrname = $(input).attr('attrname');
                    if(attrname != undefined)
                        tmp += "{attrName:'"+attrname+"',attrValue:'"+input.value+"'}";
                    else
				...
				attrs.each(function(index,select){
                    if(select.options.length > 0){
                        if(tmp.length>1)
                            tmp+=',';
                        var attrname = $(select).attr('attrname');
                        if(attrname != undefined)
                            tmp += "{attrName:'"+attrname+"',attrValue:'"+select.options[select.selectedIndex].value+"'}";
                        else
...
//	};
	$.jpolite.init(s);
	$.jpolite.gotoTab('portal');	//Activate the first tab by default, or another id of your choice
