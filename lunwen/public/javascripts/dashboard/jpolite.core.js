...
$.jpolite = {
	...
	Nav: {
		...
		cc: $("#portlet-container"),	//Content container
		...
		addStaticModule: function(m, tid){
			...
			m.tab.modules[m.uid] = m;	//Add module to tab
		},
		/**
	...
	Content: {
		...
		moduleActions: {
			...
			close: function(){
				...
				$.jpolite.Content.getRealTimeLayout();
				//$.jpolite.Content.saveLayout();
			},
			setTitle:function(text){
				$(".moduleTitle",this).text(text);
			}
		},
		...
		switchTab: function (tab_id) {
			...
			delete x.bg;
			/**
			 * change the default layotu.
			 */
			//for (var c in x){
				//this[c].attr('class', 'cc ' + x[c]);
			//}
...
//			for (var c in x){ 
//				this[c].attr('class', 'cc ' + x[c]);
//				var id = this[c].attr("id");
//				html+="<div id='"+id+"' class='cc "+x[c]+"'></div>";
//				var regex = /[last]+$/;
//				if(regex.test(x[c].trim())){
//					html+="</div>";
//					html+="<div class='row-fluid'>";
//				}
//			}
//			html = html + "</div>";
//			$("#portlet-container").html(html);
//			
//			/**
...
//			};
			...
			t.modules[m.uid] = x;
			$(x).attr('uid',m.uid);
			$(x).attr('portletId',m.id);
			var title = y.t;
			if(m.title && m.title != 'null' && m.title != '')
				title = m.title;
			$(".moduleFrame",x).attr('title',title);
			$(".moduleTitle", x).text(title);
		...
		loadLayout: function() {
			...
			window.initLayout = "{" + $.jpolite.Nav.its.map(function(){
				var t = [], m = this.modules;
				for (var i in m){	
					if (m[i].c)	{	//Skip static modules
						t.push("'".concat(m[i].id, ":", m[i].c, ":", m[i].mc, ":", m[i].mt,":",m[i].uid, "'"));
					}
				}
				return "'" + this.id + "':[" + t.toString() + "]";
			}).get().join(",") + "}";
		},
		getRealTimeLayout: function(){
			window.realTimeLayout = "{" + $.jpolite.Nav.its.map(function(){
				var t = [], m = this.modules;
				for (var i in m){	
					if (m[i].c)	{	//Skip static modules
						t.push("'".concat(m[i].id, ":", m[i].c, ":", m[i].mc, ":", m[i].mt,":",m[i].uid, "'"));
	...
	replaceModule: function(col, ids) {
		...
		this.Content.getRealTimeLayout();
